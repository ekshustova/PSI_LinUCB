...
class LinUCBwithPSI_rank1_1:
    def __init__(self, d=10, epsilon=1.0, alpha=1.0, rank=10):
        ...
        self.U = np.empty((self.d, 0), dtype=np.float32)
        self.V = np.empty((self.d, 0), dtype=np.float32)
    ...
    def _L_matvec(self, vec):
        ...
        return L_0_inv_vec - self.U.dot(self.V.T.dot(L_0_inv_vec))
    ...
    def update(self, context, reward):
        ...
        self.U, self.V = self._update_factors(bar_x=bar_x, beta_t=beta_t)
        self._update_theta()

    def _update_u_and_v(self, x_bar, beta_t):
        delta_u = beta_t * x_bar

        if self.V.shape[1] > 0:
            delta_v = x_bar - self.V @ (self.U.T @ x_bar)
        else:
            delta_v = x_bar
    ...
    def _update_factors(self, bar_x, beta_t):
        delta_U, delta_V = self._update_u_and_v(bar_x, beta_t)

        if self.U.shape[1] == 0:
            self.U = delta_U
            self.V = delta_V
            return self.U, self.V
        elif self.U.shape[1] < self.rank:
            self.U = np.column_stack([self.U, delta_U])
            self.V = np.column_stack([self.V, delta_V])
            return self.U, self.V

        if self.Ut is None:
            self.Ut, self.St, self.Vt = svd_U_V_T(self.U, self.V, self.rank)

        self.Ut, self.St, self.Vt = integrator(
            self.Ut, self.St, self.Vt, delta_U, delta_V
        )

        return self.Ut @ self.St, self.Vt

    def _update_theta(self):
        b_eps = self.epsilon * self.b

        term1 = b_eps
        term2 = self.V.dot(self.U.T.dot(b_eps))
        term3 = self.U.dot(self.V.T.dot(b_eps))
        term4 = self.V.dot(self.U.T.dot(term3))

        self.theta = term1 - term2 - term3 + term4

    def score(self, context):
        ctx = context
        mean = float(np.dot(self.theta.T, ctx))
        v = self.V.T @ ctx
        exp = self.sqrt_epsilon * np.linalg.norm(ctx - (self.U @ v))
